...
const { buildContext } = require("../security/request.context");
...
const verifyOtp = async (req, res, next) => {
  try {
    const result = await verifyRegistration(req.body, buildContext(req));
...
const login = async (req, res, next) => {
  try {
    const result = await loginUser(req.body, buildContext(req));
