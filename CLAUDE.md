...
### 🧠 AI Behaviour Engines (Maturity · Behaviour Intelligence · Healing) — CCIE extensions

Three additional engines follow the exact same CCIE pattern (config weights → `features.js` gather → pure `score()` → confidence/explain → `IntelSnapshot`). All deterministic, all confidence-hedged, none use an LLM in the score path.

- **`engines/maturity.engine.js`** (PER-USER, engine key `maturity`) — Relationship Maturity 0–100 over 10 behavioural dimensions (emotional regulation / communication / conflict resolution / trust building / consistency / empathy / accountability / respect / patience / reliability). **Not a personality test**: each dimension is computed from observable behaviour (mood moderation+recovery, message sentiment/warmth/`REPAIR_PHRASES` (rules.js), empathy = replying within 24h of a partner's visible negative mood, challenge completion, privacy openness, burst-free pacing). Unobservable dimensions are **skipped, never guessed** (`dimensionsObserved`). Works in EVERY lifecycle stage (couple dimensions degrade for solo users). Output framing: strengths / growth areas / trend — never labels or verdicts.
- **`engines/behavior.engine.js`** (COUPLE, engine key `behavior`) — reuses `gatherHealthFeatures` (zero extra queries). Reports 7 hedged **indicators** (healthy communication, emotional support, mutual effort, consistency, engagement, `conflictPressure` = INVERTED conflict frequency, `emotionalCloseness` = INVERTED emotional distance) each with an insight sentence, plus the **Attraction / Attachment / Growing-Love pattern model**: three weighted signal blends (`weights.behaviorPatterns`) normalised to a distribution; a dominant pattern is claimed only above `thresholds.behavior.patternMinDataPoints`, and the statement always says "our behavioural model currently detects stronger indicators of…" — **never facts about feelings**. Partner-order invariant (tested).
- **`engines/healing.engine.js`** (PER-USER, engine key `healing`) — Healing Progress 0–100 = **engagement with recovery activities** (routine/journaling/moodCare/challenges/sleep/support/selfDiscovery), explicitly NEVER emotional worth (mood trend can only ADD a small bonus, never punish a hard week — tested). Emits gentle `insights[]` (inactivity / withdrawal-vs-own-baseline / mood decline / heavy journaling) and, at ≥3 concurrent signals (`thresholds.healing.distressSignalCount`), a **support encouragement** pointing to trusted people / qualified professionals — never diagnostic language (tested against a clinical-word denylist).
- **Love Meter 2.0**: `relationshipHealth` gained additive components `maturity` (couple-symmetric average of both partners' latest maturity snapshots via `features.latestMaturityAvg` — null ⇒ skipped, so it stays regression-free), `emotionalSupport` (supportRatio) and `sharedGoals` (bucket planned+completed). Still identical for both partners.
- **Facade**: `getMaturity(userId)`, `getBehavior(coupleId)`, `getHealing(userId)`, `getHistorySeries(subjectId, engine, days)`. **API**: `GET /intelligence/maturity|behavior|healing` + `GET /intelligence/history/:engine?days=N` (subject is always resolved from the caller — user engines use `req.user`, couple engines the caller's couple; nobody can read another subject). `IntelSnapshot.engine` enum gained the three keys.
- **Coach continuity**: `coach.service.intelligenceContextLine` appends a hedged maturity (growing/preparing) or healing (healing) snapshot to the coach system prompt — best-effort try/catch, lazy require (no coach↔intelligence cycle), and instructs the model to treat it as a hint, never quote scores as facts.
- **Nightly cron** now also refreshes both partners' maturity + the couple behavior snapshot BEFORE the health pass, so Love Meter 2.0 reads fresh maturity and trend series never gap.
- **Frontend**: `services/intelligence.service.js`; `components/intelligence/` (`ScoreRing`, `DimensionBars`, `TrendSparkline` — token-driven, dark-mode safe, single-hue marks with text-token labels; `MaturityInsights`, `BehaviorInsights`, `HealingProgressCard` are self-fetching panels). Surfaces: route `/maturity` (lazy, NO `RequireCouple` — every stage), AI Center tabs **Signals** (behaviour) + **Maturity**, `HealingProgressCard` + a Maturity link on `HealingDashboard`, Maturity quick action on `PreparingDashboard`.
- Specs in `__tests__/behaviorEngines.test.js`: determinism, behavior partner-order invariance, attraction-vs-growing-love fixtures, no-dominant-pattern below the data floor, hedged-language asserts, healing monotonicity + non-punishment + non-clinical insight copy, Love Meter 2.0 additive regression-freeness.
